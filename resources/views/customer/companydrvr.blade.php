...
<body>
    <div>
        ...
        <div id="layoutSidenav">
            ...
            <div id="layoutSidenav_content" style="width: 650px; margin-left: 25px; margin-top: 10px; margin-bottom: 341px; margin-right: 135px;" >                 <main>
                    ...
                    <div class="container">
                        ...
                        @endif
                            ...
                            @foreach($driver as $d)
                            <div class="col-md-4">
                                <div class="card" style="width: 18rem; margin-top: 20px; margin-bottom: 20px; margin-left: 20px; margin-right: 20px">
                                    <img src="{{ asset('images/' . $d->imagename) }}" class="card-img-top" alt="{{ $d->name }}">
                                    <div class="card-body">
                                        <h5 class="card-title">Name: {{ $d->name }}</h5>
                                        <p class="card-text">NID: {{ $d->nid }}</p>
                                        {{-- <p class="card-text">Date of Birth: {{ $d->dob }}</p> --}}
                                        <p class="card-text">Email: {{ $d->email }}</p>
                                        <p class="card-text">Driver Type: {{ $d->drivertype }}</p>
                                        <p class="card-text">License: {{ $d->license }}</p>
                                        {{-- <p class="card-text">Address: {{ $d->address }}</p> --}}
                                        <p class="card-text">Qualification: {{ $d->qualification }}</p>
                                        <p class="card-text">Experience: {{ $d->experience }}</p>
                                        <p class="card-text">Verification Status:
                                            @if($d->verified == 0)
                                            <span class="fa fa-ban text-danger">Unverified Driver</span>
                                            @else
                                            <span class="fas fa-check text-success">Verified Driver</span>
                                            @endif
                                        </p>
                                        <a href="{{ url('addbooking/'.$d->id)}}" class="btn btn-primary">Book</a>
                                    </div>
                                </div>
                            </div>
                            @endforeach
